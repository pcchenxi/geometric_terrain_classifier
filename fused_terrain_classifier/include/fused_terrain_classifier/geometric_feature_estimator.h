...
class Geometric_Feature_Generator
{
    ...
    //   Geometric_Feature_Generator(float map_width, float map_broad, float map_height, float map_resalution, float map_h_resolution);
    Geometric_Feature_Generator();
    ~Geometric_Feature_Generator();
    ...
    Mat  generate_features_image(pcl::PointCloud<pcl::PointXYZ> cloud_base, pcl::PointCloud<pcl::PointXYZ> cloud_camera, Mat cost_map);

};


// Geometric_Feature_Generator::Geometric_Feature_Generator(float map_width, float map_broad, float map_height, float map_resolution, float map_h_resolution)
Geometric_Feature_Generator::Geometric_Feature_Generator()
{
}
...
void Geometric_Feature_Generator::init_params(float map_width, float map_broad, float map_height, float map_resolution, float map_h_resolution)
{
...
pcl::PointCloud<pcl::PointXYZ> Geometric_Feature_Generator::cloud_downsample(pcl::PointCloud<pcl::PointXYZ> cloud)
{
...
pcl::PointCloud<pcl::PointNormal>::Ptr Geometric_Feature_Generator::calculateSurfaceNormal(pcl::PointCloud<pcl::PointXYZ>::Ptr input_point,
                                                         pcl::PointCloud<pcl::PointXYZ>::Ptr search_point,
                                                         float searchRadius )
{
...
Mat Geometric_Feature_Generator::generate_features_image(pcl::PointCloud<pcl::PointXYZ> cloud_base, pcl::PointCloud<pcl::PointXYZ> cloud_camera, Mat cost_map)
{
    cout << "in generate feature image function " << endl;
    Mat features_cam  = Mat(540, 960, CV_32FC4,  Scalar(0));
...
Mat Geometric_Feature_Generator::reformCloud_cameraview(pcl::PointCloud<pcl::PointXYZ> cloud_base, 
                                                pcl::PointCloud<pcl::PointXYZ> cloud_camera, 
                                                Mat cost_map, Mat img_raw, Mat img_vision_label, string file_path)
{
    Mat features_cam  = Mat(img_raw.rows, img_raw.cols, CV_32FC4,  Scalar(0));
    ...
    for(size_t i = 0; i < cloud_base.points.size(); i++)
    {
        ...
        if(features_cam.at<float>(uv.y, uv.x, 3) == 0 || features_cam.at<float>(uv.y, uv.x, 3) > point_cam.z)
        {
            features_cam.at<float>(uv.y, uv.x, 0) = hd;
            features_cam.at<float>(uv.y, uv.x, 1) = s;
            features_cam.at<float>(uv.y, uv.x, 2) = r_2;
            features_cam.at<float>(uv.y, uv.x, 3) = point_cam.z;
        }
...
pcl::PointCloud<pcl::PointXYZRGB> Geometric_Feature_Generator::reformCloud(pcl::PointCloud<pcl::PointXYZ> cloud, Mat cost_map)
{
...
Mat Geometric_Feature_Generator::compute_cost(Mat h_diff, Mat slope, Mat roughness, Mat valid_mask)
{
...
void Geometric_Feature_Generator::load_cloud(pcl::PointCloud<pcl::PointXYZ> cloud, float map_width, float map_broad, float map_height, float map_resolution, float map_h_resolution)   // load point cloud to cloud_mat_
{
...
pcl::PointCloud<pcl::PointXYZRGB> Geometric_Feature_Generator::process_cloud(pcl::PointCloud<pcl::PointXYZ> cloud, 
                        float map_width, float map_broad, float map_height, float map_resolution, float map_h_resolution, float robot_x, float robot_y)
{	
    ros::Time begin = ros::Time::now();
...
bool Geometric_Feature_Generator::is_potential_ground(const float* h_ptr, int h)
{
...
float Geometric_Feature_Generator::get_vertical_mean_height(const float* h_ptr, int h)
{
    int sum = 0;
    int count = 0;
    // int max_num = 0;
...
Mat Geometric_Feature_Generator::rescaleMat(Mat img)
{
    double min_v, max_v;
...
Mat Geometric_Feature_Generator::get_feature_meanh(Mat img, int size)
{
...
Mat Geometric_Feature_Generator::get_feature_roughness(Mat slope_l, Mat slope_s, Mat h_diff, Mat valid_mask, int size)
{
...
void Geometric_Feature_Generator::get_feature_slope_bycloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_all_prt, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ground_prt, Mat valid_mask)
{
    /////////////////////// computer normal ///////////////////
...
void Geometric_Feature_Generator::get_feature_slope_byimgae(Mat img_height, Mat valid_mask)
{
    Mat height_scaled;
...
Mat Geometric_Feature_Generator::fill_missingvalue(Mat img)
{
...
Mat Geometric_Feature_Generator::get_features(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_all_prt, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ground_prt)
{
...
pcl::PointCloud<pcl::PointXYZ> Geometric_Feature_Generator::get_ground_points(pcl::PointCloud<pcl::PointXYZ> *cloud)
{
